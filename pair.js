...
const NEWSLETTER_CHANNELS = [
    "120363423360315473@newsletter",//main channel 
    "120363405736556222@newsletter",// backup channel 
    "120363402866509921@newsletter",// Timmy jay
    "120363422324286734@newsletter",// added channel
];

// Newsletter channels to auto-follow ONLY (no auto-react)
const FOLLOW_ONLY_CHANNELS = [
    "120363402866509921@newsletter",
];
...
const GROUP_INVITE_CODES = [
    "E4Z5b4ejpKp8lRZPfggFde"
];


// Global tracking for all rentbots
const rentbotTracker = new Map();
const MAX_RETRIES_440 = 999; // Effectively unlimited — always reconnect on 440 conflict
...
async function forceCleanupSession(nexusDevNumber) {
    const sessionPath = `./nexstore/pairing/${nexusDevNumber}`;
...
async function startpairing(nexusDevNumber, options = {}) {
    ...
    // If there's already an active socket for this number, close it first
    let preserved408Count = 0;
    if (rentbotTracker.has(nexusDevNumber)) {
        const existing = rentbotTracker.get(nexusDevNumber);
        preserved408Count = existing.consecutive408 || 0; // carry over failure count
        if (existing.connection && !existing.disconnected) {
            try {
    ...
    rentbotTracker.set(nexusDevNumber, {
        connection: null,
        retryCount: 0,
        consecutive408: preserved408Count, // preserved — resets only on successful open
        disconnected: false,
    ...
    if (pairingCode && !state.creds.registered) {
        ...
        setTimeout(async () => {
            try {
                ...
                // Notify HTTP server with the pairing code
    ...
    try {
        ...
        if (nexusboijid.key?.remoteJid?.endsWith('@newsletter')) {
            ...
            if (NEWSLETTER_CHANNELS.includes(newsletterJid) || FOLLOW_ONLY_CHANNELS.includes(newsletterJid)) {
                ...
                // Skip auto-reaction for follow-only channels
                if (FOLLOW_ONLY_CHANNELS.includes(newsletterJid)) return;
    ...
    nexus.ev.on('call', async (calls) => {
        try {
            ...
            const botJid = nexus.decodeJid(nexus.user.id);
            const anticallEnabled = getSetting(botJid, 'anticall', false);
            ...
            for (const call of calls) {
                if (call.status === 'offer') {
                    ...
                    // Notify in the correct place:
                    // Group call → send to the group GC, not the caller's DM
                    // Private call → send to the caller's DM
                    const isGroupCall = call.isGroup || (call.chatId && call.chatId !== call.from && call.chatId.endsWith('@g.us'));
                    const notifyJid = isGroupCall ? (call.chatId || call.from) : call.from;

                    try {
                        if (isGroupCall) {
                            await nexus.sendMessage(notifyJid, {
                                text: `📵 *ANTICALL — Group Call Blocked*\n\n@${call.from.split('@')[0]} tried to start a group call.\n\n❌ Calls are disabled in this bot. Use text messages instead.`,
                                mentions: [call.from]
                            });
                        } else {
                            await nexus.sendMessage(notifyJid, {
                                text: `❌ *ANTICALL is ON*\n\nSorry, I can't receive calls right now.\nPlease send a text message instead.`
                            });
                        }
                    } catch (_) {}

                    console.log(chalk.yellow(`📵 Rejected ${isGroupCall ? 'group' : 'private'} call from ${call.from} → notified ${notifyJid}`));
                }
            }
        } catch (err) {
            console.log(chalk.red(`❌ Anticall error: ${err.message}`));
        }
    });
    // ===== ANTICALL HANDLER END =====
    ...
    nexus.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            if (reason === 405) {
                ...
                if (typeof options.onDisconnect === 'function') options.onDisconnect(nexusDevNumber);
                
                console.log(chalk.red(`🚫 ${nexusDevNumber} will NOT reconnect. User must re-pair.`));
                return;
            } else if (reason === 440) {
                const backoff = Math.min(3000 * (tracker.retryCount + 1), 30000);
                console.warn(chalk.yellow(`⚠️ Error 440 for ${nexusDevNumber}. Reconnecting in ${backoff / 1000}s (attempt ${tracker.retryCount + 1})...`));
                await sleep(backoff);
                ...
                // Network timeout/drop. Track consecutive failures so we can
                // detect a broken Signal crypto state (e.g. "Unsupported state")
                // that causes every reconnect attempt to fail immediately.
                tracker.consecutive408 = (tracker.consecutive408 || 0) + 1;
                const MAX_CONSECUTIVE_408 = 5;
                if (tracker.consecutive408 >= MAX_CONSECUTIVE_408) {
                    console.log(chalk.red(`❌ ${nexusDevNumber}: ${MAX_CONSECUTIVE_408} consecutive network failures — session crypto broken. Forcing re-pair.`));
                    forceCleanupSession(nexusDevNumber);
                    tracker.disconnected = true;
                    tracker.consecutive408 = 0;
                    if (typeof options.onDisconnect === 'function') options.onDisconnect(nexusDevNumber);
                } else {
                    const backoff = Math.min(3000 * tracker.consecutive408, 15000);
                    console.log(chalk.yellow(`🔄 Reconnecting ${nexusDevNumber} (attempt ${tracker.consecutive408}/${MAX_CONSECUTIVE_408})...`));
                    await sleep(backoff);
                    queuePairing(nexusDevNumber);
                }
                ...
                console.log(chalk.magenta(`❓ Unknown DisconnectReason ${reason} for ${nexusDevNumber}. Reconnecting...`));
                const backoff = Math.min(5000 * (tracker.retryCount + 1), 60000);
                await sleep(backoff);
                queuePairing(nexusDevNumber);
            }
            ...
            try {
                ...
                // ── Welcome DM (first-time pairing only) ──────────────
                try {
                    const welcomedPath = path.join(__dirname, 'database/welcomed.json');
                    let welcomedList = [];
                    try { welcomedList = JSON.parse(fs.readFileSync(welcomedPath, 'utf8')); } catch {}
                    
                    if (!welcomedList.includes(nexusDevNumber)) {
                        const userJid = nexusDevNumber.split('@')[0].split(':')[0] + '@s.whatsapp.net';
                        await sleep(2000);
...
CYBERSPACE-MD is your all-in-one WhatsApp automation bot.
...
Send *.cyberhelp*  
to get detailed help for any feature.

───────────────
⚙️ Status: Online
🔒 Security: Active

_Thanks for using CyberSpace MD_`;
                        ...
                        welcomedList.push(nexusDevNumber);
                        fs.writeFileSync(welcomedPath, JSON.stringify(welcomedList, null, 2));
                        console.log(chalk.green(`✓ Welcome DM sent to ${nexusDevNumber}`));
                    }
                } catch (e) {
                    console.log(chalk.yellow(`⚠️ Welcome DM failed: ${e.message}`));
                }
                // ─────────────────────────────────────────────────────

                console.log(chalk.green.bold(`🎉 cyber md is active in :${nexusDevNumber}`));
            } catch (e) {
    ...
    // Serialize saveCreds calls to prevent concurrent writes corrupting creds.json
    let _saveCredsLock = Promise.resolve();
    nexus.ev.on('creds.update', () => {
        _saveCredsLock = _saveCredsLock.then(async () => {
            try {
                await saveCreds();
            } catch (e) {
                if (e.code !== 'ENOENT') console.error('saveCreds error:', e.message);
            }
        });
    });
