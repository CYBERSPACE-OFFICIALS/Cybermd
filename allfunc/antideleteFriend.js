...
function loadAntiSettings() {
    try {
        ...
        return JSON.parse(fs.readFileSync(ANTI_SETTINGS_FILE, 'utf8'));
    } catch {
        return { gc: false, dm: false };
    }
}
...
let globalAntiStatus = loadAntiSettings();
...
const AntiDelete = async (devtrust, updates) => {
    try {
        for (const update of updates) {
            ...
            if (!globalAntiStatus[antiType]) {
                console.log(`⏭️ Anti‑delete ${antiType} is disabled, skipping.`);
                continue;
            }
...
// ========== Toggle Anti‑Delete ==========
const toggleAntiDelete = async (devtrust, jid, type, enable = true) => {
    try {
        globalAntiStatus[type] = enable;
        saveAntiSettings(globalAntiStatus);
...
*💬 Message:* Anti‑delete for ${typeName.toLowerCase()} has been ${enable ? 'activated' : 'deactivated'}.
${enable ? 'Deleted messages will now be monitored and reported.' : 'Messages can now be deleted without tracking.'}
...
// ========== Getters ==========
const getAntiDeleteStatus = () => ({ ...globalAntiStatus, timestamp: new Date() });
